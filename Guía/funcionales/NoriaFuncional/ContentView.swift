...
struct ContentView: View {
                ...
                VStack(spacing: 40) {
                    ...
                    VStack(spacing: 10) {
                        Text("🚶‍♂️ \(distanciaHastaProximo)")
                            ...
                            .lineLimit(nil) // Permite líneas ilimitadas
    ...
    func verificarGiros() {
        ...
        distanciaHastaProximo = String(format: "Sigue adelante, hay %.0f pies hasta tu próximo giro", distancia * 3.28084)
        
        //el número de distancia es la distancia entre el diametro del vértice (usuario) y vértice
        //está en pies
        //entre menor la distancia, la indicación será dada más cerca al vértice
        if distancia < 3.0 {
...
class MapViewState: ObservableObject {
    private static var hasDrawnOnce = false
}
...
struct MapView: UIViewRepresentable {
    ...
    func updateUIView(_ mapView: MKMapView, context: Context) {
        // Borra overlays anteriores (rutas antiguas)
        mapView.removeOverlays(mapView.overlays)

        // Dibuja nueva ruta si existe
        if ruta.count > 1 {
...
class LocationManager: NSObject, ObservableObject, CLLocationManagerDelegate {
    ...
    override init() {
        ...
        locationManager.activityType = .fitness  // o .automotiveNavigation si es en carro
        locationManager.requestWhenInUseAuthorization()
